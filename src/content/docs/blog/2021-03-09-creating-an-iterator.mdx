...
This post is about a core concept in Rust, iterators. If you don't know what iterators are, please read [the chapter about iterators](https://doc.rust-lang.org/book/ch13-02-iterators.html) in the Rust book first.
...
:::tip[External and internal iteration]
...
:::tip[Why not implement `Iterator` for the collection?]
...
This doesn't work yet, because [`mem::take()`](https://doc.rust-lang.org/std/mem/fn.take.html) requires that `NodeIter` implements `Default`. But this can be fixed easily:
...
:::tip[The `mem::take()` function]
[`mem::take()`](https://doc.rust-lang.org/std/mem/fn.take.html) replaces a mutable reference with its default value and returns the previous value. The previous value is effectively _moved out_ of the reference. We use it here to convert `&mut self` to an owned value, because `parent` must be owned.
:::
...
running 1 test
[32m.[0m
test result: [32mok[0m. 1 passed; 0 failed; 0 ignored; 0 measured; 0 filtered out; finished in 0.00s
```
...
struct NodeIntoIter<It> {
    ...
    // removing elements from the front efficiently pass
...
:::note
I was told that you can use a `std::vec::IntoIter` instead of a `VecDeque`, which is more efficient, and also more idiomatic. In the same way, a `std::slice::Iter` can be used for the reference iterator.
:::
...
running 4 tests
[32m....[0m
test result: [32mok[0m. 4 passed; 0 failed; 0 ignored; 0 measured; 0 filtered out; finished in 0.00s
```
...
:::tip[How to borrow multiple things from a slice mutably?]
...
If you got stuck implementing this yourself, you may take a peek at [this playground](https://play.rust-lang.org/?version=stable&mode=debug&edition=2018&gist=936a02346b3a6703d67bf1de9f8d4cd8).
...
All code is available in this [playground](https://play.rust-lang.org/?version=stable&mode=debug&edition=2018&gist=936a02346b3a6703d67bf1de9f8d4cd8). Discussion on [Reddit](https://www.reddit.com/r/rust/comments/m0wzw7/creating_an_iterator_in_rust/).

If you have suggestions what topics I should cover next, please file a bug in the [issue tracker](https://github.com/Aloso/blog/issues). Also file a bug if you have questions or want some things explained in more detail, or if you found a mistake.
