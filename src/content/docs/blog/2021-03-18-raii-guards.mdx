...
fn main() {
    ...
    {
        // The RAII guard is created, which borrows the `RefCell` and acquires a write lock.
...
`Drop` implementations (called destructors) are called whenever the scope of the destructor's object is exited. It doesn't matter _how_ the scope is exited, e.g. it could be because of a `return`, `continue` or `break` statement, a `?` expression or a panic. There's one exception, however: When the process is exited, no destructors are run. This can be done e.g. by invoking [`abort`](https://doc.rust-lang.org/std/process/fn.abort.html), [`exit`](https://doc.rust-lang.org/std/process/fn.exit.html), or by panicking in a `Drop` implementation during another panic.

It is also possible to _leak_ objects, which means that they will never be dropped. This is usually discouraged, but allowed. Objects can be leaked e.g. with the [`Box::leak`](https://doc.rust-lang.org/std/boxed/struct.Box.html#method.leak) function, or by creating a [reference-counted](https://doc.rust-lang.org/std/rc/) graph with a cycle. Another risk are power outages and OS crashes: For obvious reasons, values aren't dropped in such cases either.
...
Discussion on [Reddit](https://www.reddit.com/r/rust/comments/m7fxaz/implementing_raii_guards_in_rust/). Please [file a bug](https://github.com/Aloso/blog/issues) if you have questions, want some things explained in more detail, or if you found a mistake. See you around!
