...
:::tip
It was pointed out that nobody actually calls them universes. The reason I thought that is the following quote from the [rustc documentation](https://doc.rust-lang.org/nightly/nightly-rustc/rustc_resolve/enum.Namespace.html):
...
:::note
Constants are usually written in UPPER_CASE, so functions and constants can't clash in idiomatic code. I wrote the above code just to prove a point, please don't quote me on it. 😉
:::

So you're probably wondering, how many namespaces are there? Let's look in the [documentation of the rustc source](https://archive.is/ajZHM)! There are 3:

- The _type namespace_
- The _value namespace_
- The _macro namespace_

Which items are part of which namespace is specified [here](https://archive.is/OZM8P) in the `DefKind` type:
...
:::note[Note about const generics]
Recently, a [MVP of const generics was stabilized](https://github.com/rust-lang/rust/blob/HEAD/RELEASES.md#version-1510-2021-03-25). This introduced an ambiguity in the parser: In the expression `foo::<X>()`, the `X` can be both a type and a value.
...
I hope you enjoyed this post, even though it's less practically useful than my [previous post](https://aloso.foo/blog/2021-03-09-creating-an-iterator).

Discussion on [Reddit](https://www.reddit.com/r/rust/comments/m1iour/rusts_universes/).

If you have suggestions what topics I should cover next, please file a bug in the [issue tracker](https://github.com/Aloso/blog/issues). Also file a bug if you have questions or want some things explained in more detail, or found a mistake.
