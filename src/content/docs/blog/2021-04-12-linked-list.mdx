...
According to [wikipedia](https://en.wikipedia.org/wiki/Persistent_data_structure):
...
running 1 test
[32m.[0m
test result: [32mok[0m. 1 passed; 0 failed; 0 ignored; 0 measured; 0 filtered out; finished in 0.00
```

## Writing an iterator

In a functional language, we'd use this data structure mainly in recursive functions. It is, after all, a recursive data structure. But since this is Rust, let's implement an [`Iterator`](https://doc.rust-lang.org/std/iter/trait.Iterator.html) for our type:
...
```ansi
[31merror[0m: the trait bound `ListIter<'_, {integer}>: DoubleEndedIterator` is not satisfied
  --> src/lib.rs:78:28
   |
 [34m2[0m |     second.into_iter().rev().copied().collect();
   |                        [31m^^^ the trait `DoubleEndedIterator` is not implemented for `ListIter<'_, {integer}>`
```
...
I'm omitting the test here, but you can read it in the [playground](https://play.rust-lang.org/?version=stable&mode=debug&edition=2018&gist=fbd969411281665b92a9884cd8d8bbbb), or you can implement it as an exercise.
...
```ansi
[31merror[E0515][0m: cannot return value referencing local variable `tail`
   --> src/lib.rs:107:5
    |
  [34m3[0m |     let v1 = tail.add(1);
    |              [34m---- `tail` is borrowed here[0m
  [34m4[0m |     let v2 = v1.add(2);
  [34m5[0m |     v2
    |     [31m^^ returns a value referencing data owned by the current function[0m

[31merror[E0515][0m: cannot return value referencing local variable `v1`
   --> src/lib.rs:107:5
    |
  [34m4[0m |     let v2 = v1.add(2);
    |              [34m-- `v1` is borrowed here[0m
  [34m5[0m |     v2
    |     [31m^^ returns a value referencing data owned by the current function
```
...
```ansi
[31merror[E0597][0m: `old_list` does not live long enough
   --> src/lib.rs:112:13
    |
  [34m1[0m | fn doesnt_work<'a>(list: &'a mut List<'a, i32>) {
    |                [34m-- lifetime `'a` defined here[0m
  [34m2[0m |     let old_list = std::mem::take(list);
  [34m3[0m |     *list = old_list.add(5);
    |             [31m^^^^^^^^[34m-------[0m
    |             [34m|[0m
    |             [31mborrowed value does not live long enough[0m
    |             [34margument requires that `old_list` is borrowed for `'a`[0m
  [34m4[0m | }
    | [34m- `old_list` dropped here while still borrowed
```
...
:::tip[What's a dangling reference?]
...
The full code, including tests, is in the [playground](https://play.rust-lang.org/?version=stable&mode=debug&edition=2018&gist=fbd969411281665b92a9884cd8d8bbbb).
